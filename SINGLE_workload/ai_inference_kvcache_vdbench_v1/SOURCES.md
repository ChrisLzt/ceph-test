...
| 访问存在偏斜但长尾仍有访问 | 对每类 KV cache 的 100 个参考 rank 施加 Zipfian skew，再聚合为40个物理bin |
...
- 当前 Zipfian 权重采用 YCSB 常用 alpha=0.99。每类 KV cache 先按100个参考
  rank计算；前20个单独保留，后80个每4个合并，形成40个物理bin。每个固定
  文件大小档按真实文件数独立计算，不进行整数化或最小1%修正；
