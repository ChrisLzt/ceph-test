...
- 设计：三个候选热点池各 100 单元，背景池 2,100 单元，容量占比约
  `4.167/4.167/4.167/87.5%`。每池以 50 个参考 rank 计算 Zipf，前 10 个
  单独保留，后 40 个每 4 个合并，最终形成 20 个物理 bin。
...
每个正式 RD 展开通配符后最多匹配 512 个 FWD；五种负载的单节点/SYSU
每阶段均分别为 120/200。MapReduce 第一/第四阶段与 HPC
