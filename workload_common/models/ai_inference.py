"""KV-cache prefill/decode/prefix lifecycle with ranked file popularity."""
...
WORKLOAD = "ai_inference_kvcache_vdbench_v1"
GROUP_UNITS = {"kv_active": 800, "kv_next": 800, "kv_prefix": 800}
RANK_COUNT = 100
...
PHASES = (
    Phase("prefill_active", "kv_active", 100, "sequential", 1),
    Phase("decode_active", "kv_active", 100, "random", 1),
    Phase("prefill_next", "kv_next", 100, "sequential", 1),
    Phase("decode_next", "kv_next", 100, "random", 1),
    Phase("prefix_reuse_primary", "kv_prefix", 100, "random", 1),
    Phase("prefix_reuse_shifted", "kv_prefix", 100, "random", 2),
)
...
def _run_text(layout: Layout, buckets: list[Bucket], grouped: dict[str, dict[int, list[Bucket]]], *, hd_line: str | None, threads: int, phase_seconds: int, fwdrate: str) -> str:
    lines = config_header("AI inference KV-cache Zipf workload - run only", hd_line=hd_line)
...
def render(*, layout: Layout, anchor_root: str, output_dir: Path, hd_line: str | None = None, threads: int = 1, phase_seconds: int = 100, fwdrate: str = "max") -> None:
    ...
    write_config(output_dir / "prepare_data.vdb", render_prepare_config("AI inference KV-cache Zipf workload", buckets, threads, hd_line=hd_line))
