...
from workload_common.models.graphchi import BIN_COUNT, RANK_COUNT, RANK_SPANS, SHARD_UNITS, STAGES, render
...
class GraphChiModelTests(unittest.TestCase):
    ...
    def test_four_shards_with_internal_zipf_ranks_render(self) -> None:
        for layout, capacity, fwd_count, total_fwd_count in (
            (SINGLE_LAYOUT, 112 * 1024 + 512, 120, 480),
            (SYSU_LAYOUT, 750 * 1024, 200, 800),
        ):
            with self.subTest(layout=layout.name), tempfile.TemporaryDirectory() as tmp:
                ...
                self.assertIn("/shard_00/rank_040/size_4m", prepare)
