...
这个负载不运行真实模型训练，也不评价 GPU/训练吞吐。它把论文和官方 benchmark 中明确描述的 AI 训练存储行为，映射成 Vdbench 对 CephFS 的阶段化访问，用于观察冷热识别。正式测试为纯读，写入只发生在造数据阶段。

- 训练数据被反复读取、过滤、复用；
- 训练数据访问存在偏斜，使用 Zipfian rank 表达“少量 rank 更频繁访问，但所有 rank 都会被访问”；
- 原始训练系统会周期性写入 checkpoint；
- 当前缩放模型在造数据时预生成 checkpoint，在正式阶段读取或复热；
...
| `checkpoint_current` | 400 | 20 MiB | 约 7.81 GiB | 造数时创建；正式阶段重复读取 |
...
`checkpoint_current` 和 `checkpoint_old` 是训练状态文件，不参与 Zipfian 训练样本分布。每组 400 个 20 MiB 文件，容量约 7.81 GiB，是当前单节点 100～120 GiB 测试预算下的缩放值。造数据阶段负责写入；正式阶段只保留近期重复读取和旧 checkpoint 恢复语义。
...
| `checkpoint_read_current_first` | 读 | `checkpoint_current` | 40s；首次加载当前 checkpoint |
| `checkpoint_read_current_second` | 读 | `checkpoint_current` | 40s；再次读取当前 checkpoint，形成近期复热 |
...
`checkpoint_read_current_first`、`checkpoint_read_current_second`、`recovery_read_old_checkpoint` 保留 MLPerf Storage checkpointing / DLIO 的加载和恢复语义。为避免单盘写入吞吐限制破坏固定 IOPS，本轮正式测试只读取造数据阶段已生成的 checkpoint；两个 current 阶段表达短期重复读取，old 阶段表达旧 checkpoint 复热。
...
```bash
ANCHOR=/mnt/cephfs/ai_training_checkpoint_vdbench_v1 THREADS=8 FORMAT_THREADS=4 DATASET_PHASE_SECONDS=160 CHECKPOINT_PHASE_SECONDS=40 FWD_RATE=1000 READ_XFER_SIZE=4m ./render_config.sh all
```
...
- 这不是训练性能 benchmark。
- 原始 benchmark 包含 checkpoint 写入；当前正式测试为消除单盘写瓶颈而采用纯读缩放模型。
