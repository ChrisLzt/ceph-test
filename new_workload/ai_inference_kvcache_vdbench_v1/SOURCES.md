...
| 访问存在偏斜但长尾仍有访问 | 对每类 KV cache 的 8 个 rank 施加 Zipfian skew |
...
- 当前 Zipfian 权重采用 YCSB 常用 alpha=0.99。脚本按 4 MiB object 计算 Zipf(0.99)，再聚合到每类 KV cache 的 8 个等容量 rank，整数化为 `79/7/4/3/2/2/2/1`；后续可由公开 trace 提取结果替换；
