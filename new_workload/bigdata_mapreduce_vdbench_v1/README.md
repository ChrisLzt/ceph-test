...
|---|---:|---:|---:|---:|
| `pool_01` | 100 | 4.6875 GiB | 50 | 20 |
| `pool_02` | 100 | 4.6875 GiB | 50 | 20 |
| `pool_03` | 100 | 4.6875 GiB | 50 | 20 |
| `background` | 2,100 | 98.4375 GiB | 50 | 20 |

每个单元含 4 个 4 MiB、2 个 8 MiB 和 1 个 16 MiB 文件。每个固定大小档
独立计算 50 个参考 rank 的 Zipf(0.99)。前 10 个参考 rank 单独保留，后 40 个
每 4 个合并，形成 20 个物理 bin。小池头部/尾部 bin 分别为 2/8 单元，背景池
分别为 42/168 单元；bin 的访问权重等于其中参考 rank 权重之和。
...
`hot_pool_01*` FWD。每阶段单节点为 120 FWD，SYSU 为 200 FWD。
...
20 个物理 bin，完成一次后可反复执行 `run_test.sh`。旧的 40-bin 数据目录不能
