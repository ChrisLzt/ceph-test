# INSPUR WRF 文件生命周期 IOR 负载

这是 150 TiB 原始容量、三副本测试套件中的 MPI 并行 HPC 备选实现。它不运行 WRF
...
三组等容量、3000 GiB、`NP=4`、阶段时长和 checkpoint 复热是冷热识别实验
...
|---|---|---:|---:|
| `startup` | `wrf_state` | 1000 GiB | 4 |
| `checkpoint` | `wrfrst_current` | 1000 GiB | 4 |
| `history` | `wrfout_current` | 1000 GiB | 4 |
| 合计 | — | 3000 GiB | 12 |
...
| `NP` | 4 | 4 个 MPI rank |
| `-F` | 启用 | 每个 rank 使用独立文件 |
| `BLOCK_SIZE` | 256000 MiB | 每 rank、每数据组 250 GiB |
| `TRANSFER_SIZE` | 4 MiB | 单次 I/O 请求 |
| `SEGMENT_COUNT` | 1 | 每 rank 一个 block |

因此每个数据组有 4 个 250 GiB 文件。IOR 文件名带 rank 后缀；prepare 与
...
| 阶段 | 时长 | 操作与数据 | 热容量口径 |
|---|---:|---|---:|
| `startup_read` | 150 s | 读取 startup 的 4 个 rank 文件 | 1000 GiB |
| `checkpoint_write` | 150 s | 覆盖写 checkpoint 的 4 个 rank 文件 | 1000 GiB |
| `history_write` | 150 s | 覆盖写 history 的 4 个 rank 文件 | 1000 GiB |
| `checkpoint_reheat` | 150 s | 读取刚写过的同一 checkpoint 文件 | 1000 GiB |
...
```bash
cd /home/chris/ceph-test/INSPUR_workload/hpc_wrf_ior_v1
./validate_model.sh
```
...
该命令会删除并重建
`/ceph-test/INSPUR_workload/hpc_wrf_ior_v1`。后续只运行：
...
```bash
ANCHOR_ROOT=/ceph-test/INSPUR_workload \
