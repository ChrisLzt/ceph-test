#!/usr/bin/env python3
"""Render the 750 GiB SYSU AI inference Vdbench workload."""
...
GROUPS = ("kv_active", "kv_next", "kv_prefix")
RANK_WEIGHTS = aggregate_zipf_percentages(64000, 20, 0.99)
PHASES = (
    ("prefill_active", "kv_active", 1, "sequential"),
    ("decode_active", "kv_active", 1, "random"),
    ("prefill_next", "kv_next", 1, "sequential"),
    ("decode_next", "kv_next", 1, "random"),
    ("prefix_reuse_primary", "kv_prefix", 1, "random"),
    ("prefix_reuse_shifted", "kv_prefix", 2, "random"),
)
...
def render_run_config(
    ...
    *,
    ...
    lines = config_header("SYSU AI inference Zipf mixed-size workload - run only", check_skew=True)
    lines.extend(render_fsd_lines(buckets))
    runs = []
    for phase, group, hot_rank, fileio in PHASES:
        names = []
...
def render(
    *,
    ...
    write_config(
        ...
        render_prepare_config("SYSU AI inference Zipf mixed-size workload", buckets, threads),
    )
    write_config(
